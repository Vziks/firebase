...
function handleTokenRefresh() {
    return FIREBASE_MESSAGING.getToken()
        .then((token) => checkSubscription(token));
}
function checkSubscription(token) {
    console.log(token)
    FIREBASE_DATABASE.ref('/tokens').orderByChild("token").equalTo(token).once('value').then((snapshot) => {
        if (snapshot.val()) {
...
function unsubscribeFromNotifications() {
    FIREBASE_MESSAGING.getToken()
        ...
        .then(() => FIREBASE_DATABASE.ref('/tokens').orderByChild("token").equalTo(token).once('value'))
        .then((snapshot) => {
